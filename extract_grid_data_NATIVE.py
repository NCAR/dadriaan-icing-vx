...
if vNWP:
  for v in p.opt['mvars']:
    if v not in ['HGT']:
      urls.append(os.path.join(p.opt['murl_pref'],mstring,'data/%s/model' % (ff),mid,'native/conus_%s' % (v)))
# Always load HGT
urls.append(os.path.join(p.opt['murl_pref'],mstring,'data/%s/model' % (ff),mid,'native/conus_HGT'))
...
for name, group in groups:
  ...
  while icnt < len(group.minI):
    ...
    if pBot == pTop:
      ...
      print(donutBot.z0.values)
      print(donutTop.z0.values)
      ...
      # If no top was found but a base was found, use the maxZ as the top
        ...
        # Construct the vertical neighborhood using the levels and dz
        # This neighborhood goes from the bottom of donutBot to the top of the grid
        finalHood = regionDS.sel(z0=list(range(int(min(donutBot.z0.values)),int(max(ncData.z0.values))+dz,dz)))
      ...
      # Both a top and a base were found
        ...
        finalHood = regionDS.sel(z0=list(range(int(min(donutBot.z0.values)),int(max(donutTop.z0.values))+dz,dz)))
    ...
    nearSub = ncData.sel(z0=list(range(int(min(finalHood.z0.values)),int(max(finalHood.z0.values))+dz,dz)))
